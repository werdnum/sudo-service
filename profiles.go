...
var (
	...
	pipefailRE       = regexp.MustCompile(`(?m)(^|[;&|]\s*)set\s+-(?:[a-zA-Z]*o\s+pipefail|[a-zA-Z]*O\s+pipefail)\b`)
	bashismRE        = regexp.MustCompile(`(^|[;&|()]\s*)\[\[|<\(|>\(|\bsource\s+`)
...
func preflightCommand(command string, profile *ExecutorProfile) ([]string, error) {
	...
	if profile != nil {
		...
		if pipefailRE.MatchString(command) && !profile.Shell.SupportsPipefail {
		...
		if bashismRE.MatchString(command) && !profile.Shell.SupportsBash {
	...
	} else if pipefailRE.MatchString(command) {
